...
def recluster_gsom(converted_feature_vector_dictionary, SF, learning_itr, smoothing_irt, temporal_contexts,
    ...
    static_recluster_start = time.time()
    excluded_time = 0
    ...
    for element in range(len(converted_feature_vector_dictionary)):
        ...
        output_loc = 'static/re-cluster/' + str(count)
        ...
        for each_item in frame_node_list:
            for frame in each_item[3]:
                if each_item[2] == farthest_clusters:
                    highlight_output = join("static/re-cluster/highlights/" + str(element) + "/" + str(each_item[2]) + "/")
                    file_path = join("static/re-cluster/highlights/" + str(element) + "/" + str(each_item[2]) + "/",
                                     str(frame) + ".jpg")
    ...
    static_recluster_end = time.time()
    print("Static Feature level 2 reclusterd: " + str(static_recluster_end - static_recluster_start - excluded_time))
